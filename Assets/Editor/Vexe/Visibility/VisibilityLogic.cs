...
namespace Vexe.Editor.Visibility
{
    public static class VisibilityLogic
    {
        ...
        {
            var attribute = CustomAttributeExtensions.GetCustomAttribute<DisplayAttribute>(member);
        ...
        public static bool IsVisibleMember(MemberInfo member)
        {
            ...
            if (field != null)
            {
                ...
                if (field.IsDefine<SerializeField>())
                    return true;
        ...
        {
            if (field.IsDefine<NonSerializedAttribute>())
                return false;

            if (field.IsLiteral)
                return false;

            if (!(field.IsPublic || field.IsDefine<SerializeField>()))
                return false;
