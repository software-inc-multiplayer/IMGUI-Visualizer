	...
	public class CategoryDefinitionResolver
	{
		...
		public CategoryDefinitionResolver()
		{
			...
			_memres = (input, def) =>
			{
				...
				{
					var caetgory = CustomAttributeExtensions.GetCustomAttribute<CategoryAttribute>(m);
					if (caetgory != null && caetgory.name == def.FullPath)
                        return true;
                    var show = CustomAttributeExtensions.GetCustomAttribute<ShowAttribute>(m);
			...
			_defres = new Func<MembersList, DefineCategoryAttribute, MembersList>[]
			{
				...
				(input, def) =>
				{
					...
					if (returnType != null)
						output.AddRange(input.Where(m => m.GetDataTyp().IsA(returnType)));
					return output;
				},
